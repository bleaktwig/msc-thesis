...
\begin{frame}{$e^-$ Statistics Study}
    ...
    After the phase space study, the second criterion to selecting a \efe{$7$ cm $v_z$} \ef{region} is maximising statistics.
    ...
    \begin{itemize}
        ...
        \item
            $e^-$ statistics are maximised between \ef{$-5$ and $2$} \efe{cm}.
    \end{itemize}
...
\begin{frame}{$e^-\pi^\pm$ Statistics Study}
    ...
    \begin{center}
        \scriptsize{\textit{\ef{$v_z$} vs. \ef{$\pi^-$} (left) and \ef{$\pi^+$} (right) statistics.}}
    \end{center}
\end{frame}
